...
const getLastChapter = async (titleId: string): Promise<number> => {
    const html: AxiosResponse | undefined = await getHtml(`https://comic.naver.com/webtoon/list?titleId=${titleId}`);
    if (html == undefined) return -1;
    const $ = load(html.data);

    const lastChapter: string[] | undefined = $("tr td.title a")?.first()?.attr("onclick")?.split("'");
...
const getWebtoon = async (titleId: string) => {
    const webtoonData: {
        ...
        author: string;
        description: string;
        contents: { title: string; thumb: string; chapter: number }[];
    } = {
        title: "웹툰 제목",
        thumb: "썸네일 링크",
        author: "작가명",
    ...
    const html = await getHtml(`https://comic.naver.com/webtoon/list?titleId=${titleId}`);
    ...
    const pageNumber: number = Math.floor(lastChapter / 10) + 1;
    const $ = load(html.data);
    ...
    for (let i = 1; i < pageNumber + 1; i++) {
        const pageHtml = await getHtml(`https://comic.naver.com/webtoon/list?titleId=${titleId}&page=${i}`);
        const $page = load(pageHtml?.data);
        $page("tbody tr").each((i, element) => {
            if ($page(element).find("td.title").text()) {
                // td 태그가 종종 광고나 다음화 링크인 경우가 있음
                const chapterURL = $page(element)?.find("td.title a")?.attr("onclick")?.split("'") ?? "";
                // nclk_v2(event,'lst.title','758037','110') 형태의 문자열을 파싱한다
...
const getBestComments = async (titleId: string, chapter: number) => {
    const bestComments: { name: string; text: string; like: string; dislike: string; date: string }[] = [];
    const browser = await puppeteer.launch();

    const page = await browser.newPage();
    await page.goto(`https://comic.naver.com/comment/comment?titleId=${titleId}&no=${chapter}`);
...
const getAllWebtoon = async () => {
    ...
    const webtoonDataList: { titleId: string; title: string; thumb: string; weekday: string }[] = [];
    const $ = load(html?.data);

    $(".thumb").each((i, element) => {
        const webtoonURL = new URL("https://comic.naver.com/" + $(element).find("a").attr("href")); // Ex) link  = '/webtoon/list?titleId=793283&weekday=tue'
